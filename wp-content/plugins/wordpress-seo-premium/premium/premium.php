...
class WPSEO_Premium {
	...
	public function add_feature_toggles( array $feature_toggles ) {
		...
		if ( $language_support->is_language_supported( $language ) ) {
			$feature_toggles[] = (object) array(
				'name'            => __( 'Идеи', 'wordpress-seo-premium' ),
				'setting'         => 'enable_metabox_insights',
				'label'           => __( 'Раздел Идеи в нашем МЕТАБОКС показывает вам полезные данные о вашем содержании, например, какие слова вы используете чаще всего.', 'wordpress-seo-premium' ),
				'read_more_label' => __( 'Узнайте больше о том, как эти идеи помогут вам улучшить содержание.', 'wordpress-seo-premium' ),
				'read_more_url'   => 'https://yoa.st/2ai',
				'order'           => 41,
			);
			$feature_toggles[] = (object) array(
				'name'            => __( 'Ссылки предложения', 'wordpress-seo-premium' ),
				'setting'         => 'enable_link_suggestions',
				'label'           => __( 'Ссылка предложения МЕТАБОКС содержит список сообщений на вашем блоге с аналогичным содержанием, которое может быть интересно ссылку на.', 'wordpress-seo-premium' ),
				'read_more_label' => __( 'Узнайте больше о том, как внутренняя компоновка может улучшить структуру сайта.', 'wordpress-seo-premium' ),
	...
	public function admin_bar_menu() {
		...
		$node = array(
			...
			'title' => __( 'Создать перенаправление', 'wordpress-seo-premium' ),
	...
	public function admin_page_meta_post_types_checkboxes( $wpseo_admin_pages, $name ) {
		Yoast_Form::get_instance()->textinput( 'page-analyse-extra-' . $name, __( 'Добавление настраиваемых полей в анализ страницы', 'wordpress-seo-premium' ) );
	}
