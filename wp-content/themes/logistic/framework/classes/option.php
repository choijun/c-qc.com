...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		extract($test = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', 'logistic' ),
			'menu_label'            => __( 'Vafpress Options', 'logistic' ),
			'priority'              => 10,
		), $configs));

		// check and set required configs
		if(isset($option_key)) $this->set_option_key($option_key);
		else throw new Exception(__( 'Option Key is required', 'logistic' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', 'logistic' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', 'logistic' ), 1);
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", 'logistic'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", 'logistic');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", 'logistic');
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> __("Successful", 'logistic'),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = __("Successful", 'logistic');	
		}
		else
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access.", 'logistic');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', 'logistic' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', 'logistic'));
			...
			$restore_section->set_title(__('Restore Default', 'logistic'));
			...
			$exim_section->set_title(__('Export/Import', 'logistic'));
