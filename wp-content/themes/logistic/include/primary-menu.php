            ...
            <div id="header" class="header-v1">
                ...
                <div id="top-search" class="clearfix search-input-unfold">
                    <form action="<?php echo home_url(); ?>/" method="get" class="wp-search-form">
                        ...
                        <input type="text" name="s" id="search" autocomplete="off" placeholder="<?php echo get_search_query() == '' ? __('Type and hit Enter', 'logistic') : get_search_query() ?>" />
                ...
                <header>
                    <nav id="top-menu" class="<?php echo esc_attr($ozy_data->menu_align);?>">
                        ...
                        <div class="primary-menu-bar-wrapper">
                        	<?php if(ozy_get_option('primary_menu_search') !== '-1') { ?>
                            <span class="menu-item-search">
                                <a href="#top-search">
                                    <span>
                                        <i class="oic-search-1">&nbsp;</i><span><?php __('Search', 'logistic') ?></span>
                                    </span>
                                </a>
							</span>
                            <?php } ?>
                            <button class="menu-toggle two"></button>
                            ...
                            <nav>
                                <?php
                                    $args = array(
                                        ...
                                        'container' => '',
                                        'fallback_cb' => true,								
                                        'walker' => new BootstrapNavMenuWalker_Overlay
                                    );
                                    if (has_nav_menu('logged-in-menu') && has_nav_menu('header-menu')) {
                                        $args['theme_location'] = (is_user_logged_in() ? 'logged-in-menu' : 'header-menu');							
                                    }else{
