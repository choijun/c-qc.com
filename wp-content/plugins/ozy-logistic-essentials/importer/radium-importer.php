 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
    	...
    	public function intro_html() {
			$memory_limit = ozy_return_bytes(ini_get('memory_limit'));
			...
			echo '	<tr><td>Memory Limit</td><td class='. ozy_check_limit_size($memory_limit, 67108864) . '>'. ($memory_limit / 1024 / 1024) .'M</td><td>64M</td></tr>';
			echo '	<tr><td>Max Execution Time</td><td class='. ozy_check_limit_size($max_execution_time, 180) . '>'. $max_execution_time .'s</td><td>180s</td></tr>';
			echo '	<tr><td>Post Max Size</td><td class='. ozy_check_limit_size($post_max_size, 32) . '>'. $post_max_size .'</td><td>32M</td></tr>';
			echo '	<tr><td>Upload Max Size</td><td class='. ozy_check_limit_size($upload_max_filesize, 32) . '>'. $upload_max_filesize .'</td><td>32M</td></tr>';					
			echo '	</tbody>';
			echo '</table>';
	    ...
	    public function demo_installer() {
			...
			if( !empty($this->imported_demos ) ) {
				...
				$button_text = esc_attr__('Step #1 - Import Demo Data', 'radium');
	       ...
	       <div class="radium-importer-wrap" data-demo-id="1"  data-nonce="<?php echo wp_create_nonce('radium-demo-code'); ?>">
			...
			echo '<p>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="corporate_business" checked>Corporate Business</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="filtered_business" checked>Filtered Business</label><br>';
			echo '<label><input type="checkbox" name="ozy_import_revo" value="microsite" checked>Microsite</label><br>';
			echo '</p>';
			
			echo '<div id="ozy_import_revo_info"></div>';
						
			echo '<p><input type="button" class="button button-primary button-large" name="ozy_import_revo_slider" id="ozy_import_revo_slider" value="Step #2 - Import Selected Revolution Sliders"></p>';
			
			echo '<br />';
			
			echo '<h3>Master Sliders</h3>';
			echo 
			'<form method="post" action="'. admin_url( 'admin.php?import=masterslider-importer&step=2' ) .'" class="msp_theme_sliders_list">';
			wp_nonce_field('msp-im-theme-sliders');
			echo 
			'<input type="hidden" name="import-theme-sliders" value="1">
			<p><label for="ms_import_theme_slider_ids"><input name="ms_import_theme_slider_ids[]" type="checkbox" value="8" checked="">Parallax Flat Slider</label><br>
			<label for="ms_import_theme_slider_ids"><input name="ms_import_theme_slider_ids[]" type="checkbox" value="6" checked="">Parallax_Slider_New</label><br>
			<label for="ms_import_theme_slider_ids"><input name="ms_import_theme_slider_ids[]" type="checkbox" value="5" checked="">Carousel Gallery</label><br>
			<label for="ms_import_theme_slider_ids"><input name="ms_import_theme_slider_ids[]" type="checkbox" value="4" checked="">Sidebar Video</label><br>
			<label for="ms_import_theme_slider_ids"><input name="ms_import_theme_slider_ids[]" type="checkbox" value="3" checked="">Timeline Slider</label><br>
			<label for="ms_import_theme_slider_ids"><input name="ms_import_theme_slider_ids[]" type="checkbox" value="1" checked="">WorlWide Map</label></p>

			<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary" value="Step #3 - Import Selected Master Sliders"></p>
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			if ( ! $fail ) {
	    				...
	    				if ( $sidebar_available ) {
	    					...
	    					$widget_message = esc_attr__( 'Imported', 'ozy-wedding_planner-essentials' );
	    				} else {
	    					$widget_message_type = 'warning';
	    					$widget_message = esc_attr__( 'Imported to Inactive', 'ozy-wedding_planner-essentials' );
	    				}
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_attr__( 'No Title', 'ozy-wedding_planner-essentials' ); // show "No Title" if widget instance is untitled
