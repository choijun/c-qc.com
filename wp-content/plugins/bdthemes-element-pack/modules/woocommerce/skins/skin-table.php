		...
		<script>
			jQuery(document).ready(function($) {
			    jQuery('#<?php echo esc_attr($id); ?>').DataTable({
		    		...
		    		'language'    : {
				        'lengthMenu' : "<?php printf(esc_html__('Show %1s Entries', 'bdthemes-element-pack'), '_MENU_' ); ?>",
				        'info'       : "<?php printf(esc_html__('Showing %1s to %2s of %3s entries', 'bdthemes-element-pack'), '_START_', '_END_', '_TOTAL_' ); ?>",
				        'search'     : "<?php esc_html_e('Search :', 'bdthemes-element-pack'); ?>",
				        'paginate'   : {
				            'previous' : "<?php esc_html_e('Previous', 'bdthemes-element-pack'); ?>",
				            'next'     : "<?php esc_html_e('Next', 'bdthemes-element-pack'); ?>",
				        },
		    		}
		    	});
			});
