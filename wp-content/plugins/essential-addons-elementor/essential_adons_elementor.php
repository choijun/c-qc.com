 ...
 * Description: Универсальная библиотека элементов для плагина компоновщика страниц Elementor для WordPress.
...
function eael_init() {
   ...
   function eael_is_failed_to_load() {
		...
		if( eael_is_elementor_active() ) {
			...
			$message = __( '<strong>Основные дополнения для Elementor</strong> требует, чтобы плагин Elementor был активным. Пожалуйста, активируйте Elementor, чтобы продолжить.', 'essential-addons-elementor' );
			$button_text = __( 'Активировать элементор', 'essential-addons-elementor' );
		} else {
			if( ! current_user_can( 'activate_plugins' ) ) {
				return;
			}
			$activation_url = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=elementor' ), 'install-plugin_elementor' );
			$message = sprintf( __( '<strong>Основные дополнения для Elementor</strong> требует %1$s"Elementor"%2$s Плагин должен быть установлен и активирован. Пожалуйста, установите Elementor, чтобы продолжить.', 'essential-addons-elementor' ), '<strong>', '</strong>' );
			$button_text = __( 'Установить Elementor', 'essential-addons-elementor' );
		}
