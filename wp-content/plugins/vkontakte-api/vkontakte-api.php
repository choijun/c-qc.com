<?php
	...
	*/
	...
	/*if( defined('WBCR_VKAPI_PLUGIN_ACTIVE') ) {
		return;
	}
	define('WBCR_VKAPI_PLUGIN_ACTIVE', true);*/

	define('WVAI_API_VERSION', '5.75');
	...
	class VK_api extends Darx_Parent {
		...
		public function __construct()
		{
			...
			if( get_option('vkapi_some_revision_d') ) {
				add_action('admin_init', create_function('', "if(!defined('WP_POST_REVISIONS'))define('WP_POST_REVISIONS',false);"));
			...
			if( is_admin() ) {
				...
				Wbcr_VkApi_Admin_Notices::instance(__FILE__);
			}
		}
...
<script>
	jQuery(document).on("click", "#vkapi_deactivate > a", function(e) {
		...
		public function cron_daily()
		{
			if( $vk_at = get_option('vkapi_at') ) {
				wp_remote_get('https://api.vk.com/method/stats.trackVisitor?v=3.0&access_token=' . $vk_at, array('user-agent' => 'Standalone'));
			}
		}
		...
		public function page_modules()
		{
			...
			<div class="wrap">
				<h1><?php _e('Social API Modules', 'vkapi');?></h1>

				<p>
					<?php _e('Модули расширяют и дополняют социальную составляющую WordPress.
					Вы всегда можете связаться с автором плагина в случае возникновения трудностей.
					Так же вы в силах запросить дополнительный функционал.', 'vkapi');?>
				</p>
					...
					<div class="darx-module-wrap">
						<div class="darx-module">
							<div class="darx-module-top">
								...
								<h3><?php _e('Comments', 'vkapi');?></h3>

								<p><?php _e('Add social comments to different post types', 'vkapi');?></p>
							</div>
							<div class="darx-module-bottom">
								<span><?php _e('<strong>Совместим</strong> с вашей версией WordPress.', 'vkapi');?></span>
					...
					<div class="darx-module-wrap">
						<div class="darx-module">
							<div class="darx-module-top">
								...
								<h3><?php _e('Crosspost', 'vkapi');?></h3>

								<p><?php _e('Featured, delayed post with photo, hashtags etc.', 'vkapi');?></p>
							</div>
							<div class="darx-module-bottom">
					...
					<div class="darx-module-wrap">
						<div class="darx-module">
							<div class="darx-module-top">
								...
								<h3><?php _e('Likes', 'vkapi');?></h3>

								<p><?php _e('Add social "likes" to wordpress post types', 'vkapi');?></p>
							</div>
							<div class="darx-module-bottom">
								<span><?php _e('<strong>Совместим</strong> с вашей версией WordPress.', 'vkapi');?></span>
								<a class="pull-right dashicons dashicons-admin-generic"
					...
					<div class="darx-module-wrap">
						<div class="darx-module">
							<div class="darx-module-top">
								...
								<h3><?php _e('Login', 'vkapi');?></h3>

								<p><?php _e('Add log in capability with social networks', 'vkapi');?></p>
							</div>
							<div class="darx-module-bottom">
								<span><?php _e('<strong>Совместим</strong> с вашей версией WordPress.', 'vkapi');?></span>
								<a class="pull-right dashicons dashicons-admin-generic"
					...
					<div class="darx-module-wrap">
						<div class="darx-module">
							<div class="darx-module-top">
								...
								<h3><?php _e('Misc', 'vkapi');?></h3>

								<p><?php _e('Some useful things', 'vkapi');?></p>
							</div>
							<div class="darx-module-bottom">
								<span><?php _e('<strong>Совместим</strong> с вашей версией WordPress.', 'vkapi');?></span>
								<a class="pull-right dashicons dashicons-admin-generic"
				...
				<p>
				<span id="stats"><?php
						printf(__('Yesterday plugin downloaded %s, %s for the week, and last month %s times.', 'vkapi'), '<b id="stats_yesterday">x</b>', '<b id="stats_week">x</b>', '<b id="stats_month">x</b>');
					?>
					<script>
						jQuery(function($) {
	...
	class VKAPI_Community extends WP_Widget {
		...
		function __construct()
		{
			...
			parent::__construct('vkapi_community', $name = 'VKapi: ' . __('Community Users', 'vkapi'), $widget_ops);
		}
...
VK.Widgets.Group('{$vkapi_divId}', {mode: {$vkapi_mode}, width: {$vkapi_width}, height: {$vkapi_height}}, {$vkapi_gid});
		...
		function form($instance)
		{
			...
			<p>
				<label for="<?php echo $this->get_field_id('gid'); ?>"><?php _e('ID of group (can be seen by reference to statistics):', 'vkapi'); ?>
					<input class="widefat"
			...
			<p>
				<label for="<?php echo $this->get_field_id('type'); ?>"><?php _e('Layout:', 'vkapi'); ?></label>
				<select name="<?php echo $this->get_field_name('type'); ?>"
				        id="<?php echo $this->get_field_id('type'); ?>"
				        class="widefat">
					<option value="users"<?php selected($instance['type'], 'users'); ?>><?php _e('Members', 'vkapi'); ?></option>
					<option value="news"<?php selected($instance['type'], 'news'); ?>><?php _e('News', 'vkapi'); ?></option>
					<option value="name"<?php selected($instance['type'], 'name'); ?>><?php _e('Only Name', 'vkapi'); ?></option>
				</select>
			</p>
		<?php
		}
...
VK.Widgets.Recommended('{$vkapi_divId}', {limit: {$vkapi_limit}, period: '{$vkapi_period}', verb: {$vkapi_verb}, target: 'blank'});
		...
		function form($instance)
		{
			...
			<p><label for="<?php echo $this->get_field_id('limit'); ?>"><?php _e('Number of posts:', 'vkapi'); ?>
					<input class="widefat"
			...
			<p>
				<label for="<?php echo $this->get_field_id('period'); ?>"><?php _e('Selection period:', 'vkapi'); ?></label>
				<select name="<?php echo $this->get_field_name('period'); ?>"
				        id="<?php echo $this->get_field_id('period'); ?>"
				        class="widefat">
					<option value="day"<?php selected($instance['period'], 'day'); ?>><?php _e('Day', 'vkapi'); ?></option>
					<option value="week"<?php selected($instance['period'], 'week'); ?>><?php _e('Week', 'vkapi'); ?></option>
					<option value="month"<?php selected($instance['period'], 'month'); ?>><?php _e('Month', 'vkapi'); ?></option>
				</select>
			</p>

			<p>
				<label for="<?php echo $this->get_field_id('verb'); ?>"><?php _e('Formulation:', 'vkapi'); ?></label>
				<select name="<?php echo $this->get_field_name('verb'); ?>"
				        id="<?php echo $this->get_field_id('verb'); ?>"
				        class="widefat">
					<option value="0"<?php selected($instance['verb'], '0'); ?>><?php _e('... people like this', 'vkapi'); ?></option>
					<option value="1"<?php selected($instance['verb'], '1'); ?>><?php _e('... people find it intersting', 'vkapi'); ?></option>
				</select>
			</p>
		<?php
		}
	...
	class VKAPI_Comments extends WP_Widget {
		...
		function form($instance)
		{
			...
			<p><label for="<?php echo $this->get_field_id('limit'); ?>"><?php _e('Number of comments:', 'vkapi'); ?>
					<input class="widefat"
					       id="<?php echo $this->get_field_id('limit'); ?>"
					       name="<?php echo $this->get_field_name('limit'); ?>"
					       type="text"
	...
	class VKAPI_Cloud extends WP_Widget {
		...
		function widget($args, $instance)
		{
			...
			$path = WP_PLUGIN_URL . '/' . dirname(plugin_basename(__FILE__)) . '/assets/js';
