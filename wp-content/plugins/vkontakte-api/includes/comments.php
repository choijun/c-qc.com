	...
	class Darx_Comments extends Darx_Parent {
		...
		public function add_page()
		{
			if( get_option('vkapi_show_comm') && get_option('vkapi_appid') ) {
				add_comments_page('Social API - ' . __('Last Comments', 'vkapi'), __('Social Comments', 'vkapi'), 'manage_options', 'darx-comments-last', array(
					$this,
					'page_comments_last'
				));
			}

			add_submenu_page('darx-modules', __('Comments Settings — Social API', 'vkapi'), __('Comments Settings', 'vkapi'), 'manage_options', 'darx-comments-settings', array(
				$this,
				'page_comments_settings'
			));
		}
		...
		public function register_settings()
		{
			...
			add_settings_section('darx-comments-base', // id
				'', // title
			...
			add_settings_field('vkapi_show_first', // id
				'', // title
			...
			add_settings_section('darx-comments-api', // id
				'', // title
				'__return_null', // callback
			...
			add_settings_section('darx-comments-media', // id
				__('Media'), // title
				'__return_null', // callback
				'darx-comments-settings-vk' // page
			);
			...
			add_settings_field('vkapi_show_comm', // id
			...
			add_settings_field('vkapi_appid', // id
				...
				array(
					...
					'type' => 'number',
					'descr' => 'Ваше приложение должно иметь тип "Веб-Сайт", введите его id в поле выше, <a href="https://www.youtube.com/watch?time_continue=87&amp;v=_HEvN0h0x0M" target="_blank">пример создания приложения</a>.',
				) // args
			);
			...
			// service key
			register_setting('darx-comments', 'vkapi_api_service_token');
			add_settings_field('vkapi_api_service_token', // id
				__('Service token', 'vkapi') . '*', // title
				array($this, 'render_settings_field'), // callback
				'darx-comments-settings-vk', // page
				'darx-comments-api', // section
				array(
					'label_for' => 'vkapi_api_service_token',
					'type' => 'text',
					'descr' => __('Этот ключ нужен для обращения к api Вконтакте. <a href="' . WVAI_PLUGIN_URL . '/images/5gdfgdfg54351231.png.">Сервисный ключ доступа</a> находится ниже поля "Секретный ключ".', 'vkapi'),
				) // args
			);
			...
			add_settings_field('vkapi_comm_height', // id
			...
			add_settings_field('vkapi_comm_width', // id
			...
			add_settings_field('vkapi_comm_limit', // id
			...
			add_settings_field('vkapi_comm_autoPublish', // id
			...
			add_settings_field('vkapi_close_wp', // id
			...
			add_settings_field('vkapi_notice_admin', // id
			...
			add_settings_field('vkapi_comm_is_postid', // id
			...
			add_settings_field('vkapi_comm_is_switcher', // id
			...
			add_settings_field('vkapi_comm_graffiti', // id
			...
			add_settings_field('vkapi_comm_photo', // id
			...
			add_settings_field('vkapi_comm_audio', // id
			...
			add_settings_field('vkapi_comm_video', // id
			...
			add_settings_field('vkapi_comm_link', // id
				__('Link', 'vkapi'), // title
				array($this, 'render_settings_field'), // callback
				'darx-comments-settings-vk', // page
				'darx-comments-media', // section
			...
			add_settings_field('fbapi_show_comm', // id
				__('Enable', 'vkapi'), // title
				array($this, 'render_settings_field'), // callback
				'darx-comments-settings-fb', // page
				'darx-comments-api', // section
			...
			add_settings_field('fbapi_appid', // id
				__('Application ID', 'vkapi'), // title
				array($this, 'render_settings_field'), // callback
				'darx-comments-settings-fb', // page
				'darx-comments-api', // section
		...
		{
			add_meta_box('vkapi_meta_box_comments', 'VKapi: ' . __('Social comments', 'vkapi'), array(
				$this,
				'render_post_meta_box_comments'
			), $page, 'advanced');
		}
		...
		{
			global $post;
			$vkapi_comm = (int)get_post_meta($post->ID, 'vkapi_comm', true);
			$fbapi_comm = (int)get_post_meta($post->ID, 'fbapi_comm', true);
		...
		public function add_vk_comments()
		{
			...
			$service_token = get_option('vkapi_api_service_token');
		...
		public function do_non_empty($count, $post_id = null)
		{
			...
			$vkapi_comm = (int)get_post_meta($post_id, 'vkapi_comm', true);
