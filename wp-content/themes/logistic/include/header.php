			...
			if ($shop_page_id > 0) { 
				...
				if(is_search()) {
					$ozy_data->_page_custom_page_title 		= __('Search results for: "', 'logistic') . get_search_query() . '"';
					$ozy_data->_page_custom_page_sub_title 	= '';
					$ozy_data->_page_hide_page_title = '0';
				}else if(is_home()) {
					$ozy_data->_page_custom_page_title 		= __('Blog', 'logistic');
					$ozy_data->_page_custom_page_sub_title 	= '';
					$ozy_data->_page_hide_page_title = '0';
				}else if(is_author()) {
					if(isset($_GET['author_name'])){$curauth = get_userdatabylogin($author_name);}else{$curauth = get_userdata(intval($author));}
					$ozy_data->_page_custom_page_title 		= __('About: ', 'logistic') . $curauth->display_name;
					$ozy_data->_page_custom_page_sub_title 	= '';
					$ozy_data->_page_hide_page_title = '0';
				}else if(is_category()) {
					$ozy_data->_page_custom_page_title 		= __('Category Archives: ', 'logistic') . '<span>' . single_cat_title( '', false ) . '</span>';
					$ozy_data->_page_custom_page_sub_title 	= '';
					$ozy_data->_page_hide_page_title = '0';
				}else if(is_archive()) {
					if ( is_day() ) : /* if the daily archive is loaded */
						$ozy_data->_page_custom_page_title = sprintf(__('Daily Archives: <span>%s</span>', 'logistic'), get_the_date() );
					elseif ( is_month() ) : /* if the montly archive is loaded */
						$ozy_data->_page_custom_page_title = sprintf( __('Monthly Archives: <span>%s</span>', 'logistic'), get_the_date('F Y'));
					elseif ( is_year() ) : /* if the yearly archive is loaded */
						$ozy_data->_page_custom_page_title = sprintf(__( 'Yearly Archives: <span>%s</span>', 'logistic'), get_the_date('Y'));
					else : /* if anything else is loaded, ex. if the tags or categories template is missing this page will load */
						$ozy_data->_page_custom_page_title = __('Blog Archives', 'logistic');
					endif;
					$ozy_data->_page_custom_page_sub_title 	= '';
					$ozy_data->_page_hide_page_title = '0';					
				}else if(is_tag()) {
					$ozy_data->_page_custom_page_title = sprintf(__( 'Tag Archives: %s', 'logistic'), '<span>' . single_tag_title( '', false ) . '</span>');
					$ozy_data->_page_custom_page_sub_title 	= '';
					$ozy_data->_page_hide_page_title = '0';					
				}else{
					if(isset($post->ID)) {
				...
				if ( have_posts() ) while ( have_posts() ) : the_post();
					...
					if ( has_post_thumbnail() ) { 
						...
						if(isset($src[0])) {
							...
							echo '					<p class="g">' . __('By ', 'logistic') . '</p>';
							echo '					<p>'; the_author_posts_link(); echo '</p>';
							echo '					<p class="g">' . __(' in ', 'logistic') . '</p>';
							echo '					<p>'; the_category(', '); echo '</p>';
							echo '					<p class="g">' . __('Posted ', 'logistic') . '</p>';
							echo '					<p>'; the_time('F j, Y'); _e(' at ', 'logistic'); the_time(); echo '</p>';
							echo '				</div>';
							echo '			</div>
										</div>';
